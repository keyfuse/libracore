...
// GenerateKeyPair is a wrapper for ecdsa.GenerateKey that returns a
...
func NewKeyFromSeed(seed []byte) (*PrivateKey, *PublicKey, error) {
	...
	curve := Edwards()
	bInBig := new(big.Int).SetBytes(h[:])
	bInBig.Mod(bInBig, curve.N)
	b1 := copyBytes(bInBig.Bytes())
	b1[31] &= 248

	return PrivKeyFromScalar(b1[:])
}
