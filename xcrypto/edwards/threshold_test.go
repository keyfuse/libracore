...
func TestSchnorrThreshold(t *testing.T) {
	...
	for i := 0; i < numTests; i++ {
		...
		publicNonceSum := PubkeyAdd(pubNoncesToUse)
		cmp := publicNonceSum != nil
		if !cmp {
			t.Fatalf("expected %v, got %v", true, cmp)
		}
		for j := range keysToUse {
			r, s, err := PartialSign(msg, keysToUse[j], allPksSum, privNoncesToUse[j], publicNonceSum)
			if err != nil {
				t.Fatalf("unexpected error %s, ", err)
			}
		...
		// Combine signatures.
		combinedSignature, err := CombinePartialSigs(partialSignatures)
		if err != nil {
			t.Fatalf("unexpected error %s, ", err)
		}
		...
		for j := range keysToUse {
			...
			publicNonceSum := PubkeyAdd(localPubNonces)
